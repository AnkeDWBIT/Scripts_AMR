...
output_file = "/home/guest/BIT11_Traineeship/Ecoli_AMR/Reference_lists/BN_reflist.xlsx"

# STEP 1 : Retrieve data from the input_dir and save it in a dictionary
############################################################################################################################################################
# Loop through the input directory and look for the files named "Resistance_ResultsTableAcq.tsv" & "Resistance_ResultsTableMut.tsv"
data_dict = {}
        ...
        if file == "Resistance-ResultsTableMut.tsv":
            ...
            with open(file_path, 'r') as file:
                ...
                for i, line in enumerate(lines):
                    ...
                    gene = parts[1].strip()
                    antibiotic = parts[0].strip()
                    # Add the gene & antibiotic to the dictionary
                    if gene not in data_dict:

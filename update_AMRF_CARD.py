...
while row <= AMRF_ws.max_row:
    ...
    # Write these values to specific cells in the output file
    output_ws.cell(row=row, column=6).value = value_2
    output_ws.cell(row=row, column=13).value = value_3
    output_ws.cell(row=row, column=20).value = value_4
    output_ws.cell(row=row, column=27).value = value_5
    output_ws.cell(row=row, column=34).value = value_6
    output_ws.cell(row=row, column=41).value = value_7
    output_ws.cell(row=row, column=48).value = value_8
    output_ws.cell(row=row, column=58).value = value_9
    output_ws.cell(row=row, column=62).value = value_10
    output_ws.cell(row=row, column=69).value = value_12
    output_ws.cell(row=row, column=76).value = value_13
    output_ws.cell(row=row, column=83).value = value_14
    # Only if row 15 (trimethoprim) & 16 (sulfamethoxazole) are both "R", then write "R" to the output file
    if value_15 == "R" and value_16 == "R":
